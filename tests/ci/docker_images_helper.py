...
def pull_image(
    image: Union[DockerImage, str], tag: Optional[str] = None
) -> DockerImage:
    tag = tag or "latest"
    docker_image = DockerImage(image, tag) if isinstance(image, str) else image
    try:
        logging.info("Pulling image %s - start", docker_image)
        subprocess.check_output(
            f"docker pull {image}",
            stderr=subprocess.STDOUT,
            shell=True,
        )
        logging.info("Pulling image %s - done", docker_image)
...
def get_docker_image(image_name: str) -> DockerImage:
    ...
    if "{" in DOCKER_TAG:
        # DOCKER_TAG is json string with "image_name - tag" mapping
